...
namespace Arcane.Xna.Presentation
{
    public class GraphicsDeviceManager : IGraphicsDeviceService, IDisposable, IGraphicsDeviceManager
    {
        ...
        {
            depthFormatsWithStencil = new DepthFormat[] { DepthFormat.Depth24Stencil8, DepthFormat.Depth24, DepthFormat.Depth24, DepthFormat.Depth24Stencil8 };
        ...
        private void AddDevices(GraphicsAdapter adapter, DisplayMode mode, GraphicsDeviceInformation baseDeviceInfo, List<GraphicsDeviceInformation> foundDevices)
        {
            ...
            adapter.QueryBackBufferFormat(item.GraphicsProfile, mode.Format, this.PreferredDepthStencilFormat, this.PreferMultiSampling ? 0x10 : 0, out format, out format2, out num);
            item.PresentationParameters.BackBufferFormat = format;
            item.PresentationParameters.DepthStencilFormat = format2;
            item.PresentationParameters.MultiSampleCount = num;
                ...
                if (!forceCreate && (this.device != null))
                {
                    ...
                    if (this.CanResetDevice(graphicsDeviceInformation))
                    {
                        try
                        {
                            ...
                            device.Reset(information2.PresentationParameters, information2.Adapter);
                            flag2 = false;
                        }
                        catch
                        {
        ...
        private void CreateDevice(GraphicsDeviceInformation newInfo)
        {
            ...
            GraphicsDevice device = new GraphicsDevice(newInfo.Adapter, newInfo.GraphicsProfile, newInfo.PresentationParameters);
            this.device = device;
